...
gitWatermark <- function(VersionNumber = TRUE, URL = TRUE,
                         timeAndDate = TRUE, comment = '',
                         x = 0.5, y = 0.5,
                         col = 'black',transparency = 0.5, srt = 45, allPlot = TRUE, 
                         waterMarkCex = 2,...) {

    txt = gitFullInfo(VersionNumber, URL, timeAndDate, comment)
    
    if (allPlot) par(fig = c(0, 1, 0, 1), mar = rep(0, 4))
    ...
    try(text(x, y, txt, srt = srt, col = col, xpd = NA,cex = waterMarkCex,...), silent = TRUE)
